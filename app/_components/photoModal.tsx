...
export default function PhotoModal({ photo, onClose }: { photo: Photo | null; onClose: () => void }) {
    ...
    const photoHasMetadata = photo.title || photo.description || photo.yearTaken || (photo.tags && photo.tags.length > 0);
    const titleOnly = photo.title && !photo.description && !photo.yearTaken && (!photo.tags || photo.tags.length === 0);
